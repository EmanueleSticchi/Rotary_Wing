classdef Elica2
    ...
    % Aerodinamica
    % ---------------------------------------------------------------------
        Cl= @(alpha,M,Re,r_bar) 2*pi*alpha;
        Cd= @(alpha,M,Re,r_bar) 0.01*alpha./alpha;
...
% ---------------------------------------------------------------------
    methods
        ...
        function F = ffunc(obj,x,idx)
            ...
            F(1) = alpha   - (obj.theta(idx) - phi);
            F(2) = phi     - atan2(V_inf*(1+a),(obj.omega*obj.r_bar(idx)*(1 - ap)));
            F(3) = lambda1 - (obj.Cl(alpha,M,Re,obj.r_bar(idx))*cos(phi)...
                            -obj.Cd(alpha,M,Re,obj.r_bar(idx))*sin(phi));
            F(4) = lambda2 - (obj.Cl(alpha,M,Re,obj.r_bar(idx))*sin(phi)...
        ...
        function obj=BEMT(obj,J,options)
        ...
        for jdx=1:length(J)
            for idx =1:obj.n_r
                ...
                % x = [alpha phi lambda1 lambda2 a a' J]
                
                x0 = x;  % initialize solution
                % assign J
                b(n)=J(jdx);
                [x,fval] = fmincon(@(x) ffunc(obj,x,idx), ... % objective func.
                    x0, ...           % initial guess
            ...
            disp(['Complete = ',num2str(jdx/length(J)*100),'%'])
        end
        s.J=J;
        s.options=options;
